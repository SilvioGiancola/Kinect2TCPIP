...
public:
    ...
    Eigen::Affine3f getAffine3f(){return Eigen::Affine3f(matrix4());}
    ...
    void setOrigin4(Eigen::Vector4f orig){_transformation.matrix().block(0,3,3,1) = orig.head(3);}
    ...
    {
        return Transform(this->matrix4() * T.matrix4());
    }


    Transform premultiplyby(Transform T)
    {
        return Transform(T.matrix4() * this->matrix4());
    }
